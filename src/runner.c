...
get_io_flags(enum io_operator io_op)
{
  ...
  switch (io_op) {
    ...
    case OP_GREAT:    /* > */
      ...
      break;
    case OP_DGREAT: /* >> */
      flags = O_APPEND | O_CREAT | O_WRONLY;  /* TODO */
      break;
    case OP_LESSGREAT: /* <> */
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
    ...
    if (child_pid == 0) {
      if (is_builtin) {
         ...
         */
        if (upstream_pipefd >= 0) {
            if (dup2(upstream_pipefd, STDIN_FILENO) < 0) {
                 goto err;
            }
            close(upstream_pipefd); // Close the original file descriptor
        }
        ...
        if (downstream_pipefd >= 0) {
            ...
            close(downstream_pipefd); // Close the original file descriptor
        }
